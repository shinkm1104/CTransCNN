...
from model.core import add_multi_label_auc
...
class MultiLabelDataset(BaseDataset):
    ...
    def evaluate(self,
        ...
        #                    'avg_zero_one_loss']
        allowed_metrics = ['mAP', 'CP', 'CR', 'CF1', 'OP', 'OR', 'OF1', 'multi_auc', 'hamming_loss', 
                           'ranking_loss', 'multilabel_accuracy', 'multilabel_coverage', 'one_error', 
                           'subset_accuracy', 'macro_f1', 'micro_f1', 'all']
        eval_results = {}
        ...
        # 'all' 메트릭이 있으면 모든 메트릭으로 교체
        if 'all' in metrics:
            metrics = [m for m in allowed_metrics if m != 'all']
        ...
        if len(set(metrics) - {'mAP'}) != 0:
            ...
            #                     'avg_zero_one_loss']
            performance_keys = ['CP', 'CR', 'CF1', 'OP', 'OR', 'OF1', 'multi_auc', 'hamming_loss', 
                                'ranking_loss', 'multilabel_accuracy', 'multilabel_coverage', 
                                'one_error', 'subset_accuracy', 'macro_f1', 'micro_f1']
